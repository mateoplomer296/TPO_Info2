...
#define DAC1                ( (DAC_Type *) DAC0_BASE)
...
// 1. Estructura de la API del FRO
typedef struct {
	const uint32_t reservados1;
	const uint32_t reservados2;
    void (*set_fro_frequency)(uint32_t iFreq);
} FRO_API_T;


typedef struct {
    // (Ocupan offsets 0x00, 0x04, 0x08,)
    const uint32_t reservados_iniciales[3];

    // La API de FRO (Offset 0xC)
    volatile const FRO_API_T *froApiBase;


} LPC_ROM_API_T;

// 4. El anclaje a la dirección física
#define LPC_ROM_API (*(LPC_ROM_API_T ** )0x0F001FF8)
...
const uint8_t IOCON_INDEX_PIO0[] =
{17,11,6,5,4,3,16,15,4,13,8,7,2,1,18,10,9,0,30,29,28,27,26,25,24,23,22,21,20,0,0,35};


const uint8_t IOCON_INDEX_PIO1[] =
{36,37,3,41,42,43,46,49,31,32,55,54,33,34,39,40,44,45,47,48,52,53,0,0,0,0,0,0,0,50,51,35};
/////////////////////////////
